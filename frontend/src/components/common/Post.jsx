...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(data.error || "Cannot delete the post");
        }
        return data;
      } catch (error) {
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    ...
    onSuccess: (updatedLikes) => {
      /*
      The below fn can be used too, but it again refetches the whole posts which isn't best UX.
      queryClient.invalidateQueries({ queryKey: ["posts"] });
      */
